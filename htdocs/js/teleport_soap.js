...
function SOAP(){
	...
	this.doSerialize = function(args, name){
		...
		if(Kernel.isFalse(c))
			...
			return '<' + name + ' ' + this.getXSIType(c) + '>' + this.serialize[c.dataType || "object"](c) + '</' + name + '>';
	}
	...
	this.unserialize = function(data){
		...
		switch(type){
			...
			default:
				//Custom Type
				if(type && !self[type]) throw new Error(0, "Invalid Object Specified in SOAP message: " + type);
	...
	this.checkErrors = function(data, http){
		...
		data = data.getElementsByTagName("SOAP-ENV:Body")[0];///node()/node()[2]
