...
function JSON(){
	...
	this.serialize = {
		...
		object : function(o){
			var str = [];
			for(var prop in o){
				str.push('"' + prop + '": ' + this.host.doSerialize(o[prop]));
			}
		...
		array : function(a){
			for(var q=[],i=0;i<a.length;i++)
				q.push(this.host.doSerialize(a[i]));
	...
	this.getSerialized = function(functionName, args){
		...
		this.id++;
  		...
  		var message = '{"method":"' + functionName + '","params":' + this.doSerialize(args) + ',"id":' + this.id + '}';
  		return message;
  	}
	...
	this.checkErrors = function(data, http){
		...
		//ERROR CHECK??
		if(data.firstChild.tagName == "fault"){
		...
		data = data.selectSingleNode("params/param/node()");
