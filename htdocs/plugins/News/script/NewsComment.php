 ...
 * @package Common
 */
class NewsComment extends Observer
{
	const VIEW_KEY = 'com';
	const VIEW_ADD = 1;
	...
	public function __construct($plugin)
	{
		...
		$this->template = array();
		$this->templateFile = "newscomment.tpl";
		$this->basePath = realpath(dirname(__FILE__)."/../")."/";

		$this->sqlParser->setSelect('select');
		$this->sqlParser->setTable('news_comment', 'a');
		$this->sqlParser->addField(new SqlField('a', 'com_id', 'id', 'id', SqlParser::getTypeSelect()|SqlParser::PKEY, SqlField::TYPE_INTEGER));
		$this->sqlParser->addField(new SqlField('a', 'com_news_id', 'news_id', 'News id', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER, true));
		...
		$this->sqlParser->addFrom("inner join news as b on b.news_id = a.com_news_id");
...
/*----- handle http requests {{{ -------*/
	...
	public function handleHttpPostRequest()
	{
		...
		{
			case News::VIEW_COMMENT_OVERVIEW : $this->handleTreeOverview(); break;
			case News::VIEW_COMMENT_EDIT : $this->handleTreeEditPost(); break;
			case News::VIEW_COMMENT_DELETE : $this->handleTreeDeletePost(); break;
	...
	public function handleHttpGetRequest()
	{
		...
		{
			case News::VIEW_COMMENT_OVERVIEW : $this->handleTreeOverview(); break;
			case News::VIEW_COMMENT_EDIT : $this->handleTreeEditGet(); break;
			case News::VIEW_COMMENT_DELETE : $this->handleTreeDeleteGet(); break;
			default : $this->handleOverview(); break;
		}
	}
//}}}
...
/*------- overview request {{{ -------*/
	...
	*/
	...
	public function getOverview($news_id)
	{
		...
		if(!$template->isCached())
		{
			...
			$template->setVariable('newssettings',  $settings);
...
/*------- tree overview request {{{ -------*/
	...
	*/
	private function handleTreeOverview()
	{
		...
		$view->setType(News::VIEW_COMMENT_OVERVIEW);
		...
		// create urls
		$url_edit = clone $url;
		$url_edit->setParameter($view->getUrlId(), News::VIEW_COMMENT_EDIT);

		$url_del = clone $url;
		$url_del->setParameter($view->getUrlId(), News::VIEW_COMMENT_DELETE);
...
/*------- tree settings {{{ -------*/
	private function handleTreeSettings($template)
	{
		...
		$url->setParameter('news_id', $news_id);
		$url->setParameter('tag', $tag);
		$url->setParameter($view->getUrlId(), News::VIEW_COMMENT_OVERVIEW);
		$template->setVariable('href_back',  $url->getUrl(true), false);

		// add breadcrumb item
		$breadcrumb = array('name' => $view->getName(News::VIEW_COMMENT_OVERVIEW), 'path' => $url->getUrl(true));
	...
	*/
	public function addComment($values)
	{
		...
		try
		{
			...
			$news = $this->plugin->getObject(News::TYPE_DEFAULT);
			$newsDetail = $news->getDetail(array('id' => $values['news_id']));

			$url = new Url();
			$url->setPath($this->director->tree->getPath($newsDetail['tree_id']));
			$url->setParameter('id', $newsDetail['id']);
			$url->setParameter($view->getUrlId(), News::VIEW_DETAIL);

			// notify insert
			$ip = $request->getValue('REMOTE_ADDR', Request::SERVER);
			$template = new TemplateEngine($this->getPath()."templates/newscommentemail.tpl");
			$template->setVariable($values);
			$template->setVariable('newsName', $newsDetail['name']);
			...
			$this->director->systemUser->notify($newsDetail['tree_id'], 'Comment added at '.$request->getDomain(), $template->fetch());
...
/*------- tree edit request {{{ -------*/
	...
	*/
	private function handleTreeEditGet($retrieveFields=true)
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(News::VIEW_COMMENT_EDIT);
...
/*------- tree delete request {{{ -------*/
	...
	*/
	private function handleTreeDeleteGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(News::VIEW_COMMENT_DELETE);
	...
	private function handleTreeDeletePost()
	{
		...
		try 
		{
			...
			viewManager::getInstance()->setType(News::VIEW_COMMENT_OVERVIEW);
