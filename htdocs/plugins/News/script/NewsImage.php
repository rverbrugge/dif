 ...
 * @package Common
 */
class NewsImage extends Observer
{
	...
	public function __construct($plugin)
	{
		...
		$this->template = array();
		$this->templateFile = "newsimage.tpl";
		$this->basePath = realpath(dirname(__FILE__)."/../")."/";

		$this->sqlParser->setSelect('select');
		$this->sqlParser->setTable('news_image', 'a');
		$this->sqlParser->addField(new SqlField('a', 'img_id', 'id', 'id', SqlParser::getTypeSelect()|SqlParser::PKEY, SqlField::TYPE_INTEGER));
		$this->sqlParser->addField(new SqlField('a', 'img_news_id', 'news_id', 'Nieuwsbericht', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER, true));
		...
		$this->sqlParser->addFrom("inner join news as b on b.news_id = a.img_news_id");
...
/*-------- DbConnector insert function {{{------------*/
	...
	private function getNextWeight()
	{
		...
		$retval = 0;
		$offset = 10;
		$query = sprintf("select max(img_weight) from news_image where img_news_id = %d", $news_id);
   ...
   *
	...
	protected function insertImage($id, $values)
	{
		...
		$db = $this->getDb();
		$query = sprintf("update news_image set img_image= '%s', img_thumbnail = '%s', img_img_x = %d, img_img_y = %d, img_img_width = %d, img_img_height = %d where img_id = %d", 
   ...
   *
	...
	protected function handlePreUpdate($id, $values)
	{
		...
		$db = $this->getDb();
		$query = "update news_image set img_image = '', img_thumbnail = '' where img_id = {$id['id']}";
		$res = $db->query($query);
		if($db->isError($res)) throw new Exception($res->getDebugInfo());
	}
...
/*----- handle http requests {{{ -------*/
	...
	public function handleHttpPostRequest()
	{
		...
		{
			case News::VIEW_IMAGE_OVERVIEW : $this->handleTreeOverview(); break;
			case News::VIEW_IMAGE_NEW :  $this->handleTreeNewPost(); break;
			case News::VIEW_IMAGE_EDIT : $this->handleTreeEditPost(); break;
			case News::VIEW_IMAGE_DELETE : $this->handleTreeDeletePost(); break;
			case News::VIEW_IMAGE_IMPORT : $this->handleImportPost(); break;
			case News::VIEW_IMAGE_RESIZE : $this->handleResizePost(); break;
	...
	public function handleHttpGetRequest()
	{
		...
		{
			case News::VIEW_IMAGE_OVERVIEW : $this->handleTreeOverview(); break;
			case News::VIEW_IMAGE_NEW : $this->handleTreeNewGet(); break;
			case News::VIEW_IMAGE_EDIT : $this->handleTreeEditGet(); break;
			case News::VIEW_IMAGE_DELETE : $this->handleTreeDeleteGet(); break;
			case News::VIEW_IMAGE_IMPORT : $this->handleImportGet(); break;
			case News::VIEW_IMAGE_RESIZE : $this->handleResizeGet(); break;
			case News::VIEW_DETAIL : $this->handleOverview(); break;
...
/*----- handle cli requests {{{ -------*/
	...
	public function handleCliRequest($cliServer)
	{
		...
		if(!$cliServer->parameterExists('news_id') && !$force) throw new Exception("Parameter news_id not set. Use --force to force resize without news\n");
	...
	private function resize($tree_id=NULL, $tag=NULL, $news_id=NULL, $stdout=true)
	{
		...
		if(isset($news_id)) $searchcriteria['news_id']	= $news_id;
...
/*------- tree overview request {{{ -------*/
	...
	*/
	private function handleTreeOverview()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(News::VIEW_IMAGE_OVERVIEW);

		if(!$request->exists('news_id')) throw new Exception('Nieuwsbericht ontbreekt.');
		...
		// create urls
		$url_new = clone $url;
		$url_new->setParameter($view->getUrlId(), News::VIEW_IMAGE_NEW);
		$template->setVariable('href_new',  $url_new->getUrl(true), false);

		$url_import = clone $url;
		$url_import->setParameter($view->getUrlId(), News::VIEW_IMAGE_IMPORT);
		$template->setVariable('href_import',  $url_import->getUrl(true), false);

		$url_resize = clone $url;
		$url_resize->setParameter($view->getUrlId(), News::VIEW_IMAGE_RESIZE);
		$template->setVariable('href_resize',  $url_resize->getUrl(true), false);

		$url_edit = clone $url;
		$url_edit->setParameter($view->getUrlId(), News::VIEW_IMAGE_EDIT);

		$url_del = clone $url;
		$url_del->setParameter($view->getUrlId(), News::VIEW_IMAGE_DELETE);
...
/*------- tree settings {{{ -------*/
	private function handleTreeSettings($template)
	{
		...
		$url->setParameter('tag', $tag);
		$url->setParameter($view->getUrlId(), News::VIEW_IMAGE_OVERVIEW);
		$template->setVariable('href_back',  $url->getUrl(true), false);

		// add breadcrumb item
		$breadcrumb = array('name' => $view->getName(News::VIEW_IMAGE_OVERVIEW), 'path' => $url->getUrl(true));
...
/*------- tree new request {{{ -------*/
	...
	*/
	private function handleTreeNewGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(News::VIEW_IMAGE_NEW);
	...
	private function handleTreeNewPost()
	{
		...
		try 
		{
			...
			viewManager::getInstance()->setType(News::VIEW_IMAGE_OVERVIEW);
...
/*------- tree edit request {{{ -------*/
	...
	*/
	private function handleTreeEditGet($retrieveFields=true)
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(News::VIEW_IMAGE_EDIT);
...
/*------- tree delete request {{{ -------*/
	...
	*/
	private function handleTreeDeleteGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(News::VIEW_IMAGE_DELETE);
...
/*------- import request {{{ -------*/
	...
	*/
	private function handleImportGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(News::VIEW_IMAGE_IMPORT);
...
/*------- resize request {{{ -------*/
	...
	*/
	private function handleResizeGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(News::VIEW_IMAGE_RESIZE);
		$auth = Authentication::getInstance();
