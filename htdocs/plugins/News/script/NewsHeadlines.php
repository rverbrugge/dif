 ...
 * @package Common
 */
class NewsHeadlines extends Observer
{
	...
	public function __construct($plugin)
	{
		...
		$this->template = array();
		$this->templateFile = "newsheadlines.tpl";
		$this->basePath = realpath(dirname(__FILE__)."/../")."/";

		$this->sqlParser->setSelect('select');
		$this->sqlParser->setTable('news_headlines', 'a');
		...
		$this->sqlParser->addField(new SqlField('a', 'news_name', 'name', 'Naam', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING));
		...
		$this->sqlParser->addField(new SqlField('a', 'news_display', 'display', 'Weergave', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER, true));
...
/*-------- helper functions {{{----------*/
	...
	private function getNewsOverview()
	{
		if(isset($this->newsOverview)) return $this->newsOverview;

		require_once('NewsOverview.php');
		$this->newsOverview = new NewsOverview($this->plugin);
		return $this->newsOverview;
	}
...
/*-------- DbConnector insert function {{{------------*/
	...
	public function getDefaultValue($fieldname)
	{
		switch($fieldname)
		{
			...
			case 'display' : return News::DISP_FULL; break;
...
/*------- tree edit request {{{ -------*/
	...
	*/
	private function handleTreeEditGet($retrieveFields=true)
	{
		...
		$searchcriteria = array('classname' => 'News',
														'plugin_type'			=> News::TYPE_DEFAULT);
