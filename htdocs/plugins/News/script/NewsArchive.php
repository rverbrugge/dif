 ...
 * @package Common
 */
class NewsArchive extends Observer
{
	...
	public function __construct($plugin)
	{
		...
		$this->template = array();
		$this->templateFile = "newsarchive.tpl";
		$this->basePath = realpath(dirname(__FILE__)."/../")."/";
		$this->reference = array();

		$this->sqlParser->setSelect('select');
		$this->sqlParser->setTable('news_archive', 'a');
		...
		$this->sqlParser->addField(new SqlField('a', 'news_online', 'online', 'Online datum', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_DATE));
...
/*-------- helper functions {{{----------*/
	...
	{
		if(array_key_exists('NewsAttachment', $this->reference)) return $this->reference['NewsAttachment'];

		require_once "NewsAttachment.php";
		$attachment = new NewsAttachment($this->plugin);
		$this->reference[$attachment->getClassName()] = $attachment;
		return $attachment;
	}
