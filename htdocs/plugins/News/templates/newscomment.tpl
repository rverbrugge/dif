...
<?  /*-----tree overview {{{------*/
if($currentView == News::VIEW_COMMENT_OVERVIEW):?>
<p class="options">
...
<input type="hidden" name="<?=ViewManager::getInstance()->getUrlId();?>" value="<?=News::VIEW_COMMENT_DELETE;?>" />
...
<?  /*-----tree edit {{{------*/
if($currentView == News::VIEW_COMMENT_EDIT):?>
...
<?  /*-----delete {{{------*/
if($currentView == News::VIEW_COMMENT_DELETE):?>
...
<?  /*-----overview {{{------*/
if($currentView == News::VIEW_DETAIL || $currentView == ''):?>

<div id="commenttag">
<?if($newssettings['comment_display'] == NewsComment::DISP_FORM_BOTTOM):?>
...
<?if($newssettings['comment_title']):?><h2 id="cmttitle"><?=$newssettings['comment_title'];?></h2><?endif;?>

<?if(isset($commentError)):?><p class="error"><?=$commentError;?></p><?endif;?>

<div id="cmtadd">
<!--form action="<?=$urlPath;?>" method="post" enctype="multipart/form-data">
<input type="hidden" name="<?=NewsComment::VIEW_KEY;?>" value="<?=NewsComment::VIEW_ADD;?>" />
<input type="hidden" name="id" value="<?=$news['id'];?>" />
<input type="hidden" name="news_id" value="<?=$news['id'];?>" /-->


<table>
	<tr><th><?=$newssettings['cap_name'];?></th>				<td><input id="comm_name" type="text" name="name" value="<?=isset($cmtValues)?$cmtValues['name']:'';?>" size="<?=$newssettings['comment_width'];?>" /></td></tr>
<?if($newssettings['cap_email']):?>
	<tr><th><?=$newssettings['cap_email'];?></th>				<td><input id="comm_email" type="text" name="email" value="<?=isset($cmtValues)?$cmtValues['email']:'';?>" size="<?=$newssettings['comment_width'];?>" /></td></tr>
<?endif;?>
	<tr><th><?=$newssettings['cap_desc'];?></th>	<td><textarea id="comm_text" name="text" rows="4" cols="<?=$newssettings['comment_width'];?>"><?=isset($cmtValues)?$cmtValues['text']:'';?></textarea></td></tr>
</table>

<input type="button" value="<?=$newssettings['cap_submit'];?>" class="formbutton" onclick="addComment(<?=$news['id'];?>, document.getElementById('comm_name').value, document.getElementById('comm_text').value)" />
<!--/form-->
</div>

<?if($newssettings['comment_display'] == NewsComment::DISP_FORM_TOP):?>
<?if($comment['page_numbers']['total'] > 1):?><p class="pagenav"><?=$comment['totalItems'];?> posts</p><?endif;?>
