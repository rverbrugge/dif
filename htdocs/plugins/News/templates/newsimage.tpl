...
<?  /*-----tree overview {{{------*/
if($currentView == News::VIEW_IMAGE_OVERVIEW):?>
<p class="options">
...
<input type="hidden" name="<?=ViewManager::getInstance()->getUrlId();?>" value="<?=News::VIEW_IMAGE_DELETE;?>" />
...
<?  /*-----tree new and edit {{{------*/
if($currentView == News::VIEW_IMAGE_NEW || $currentView == News::VIEW_IMAGE_EDIT):?>
...
<?  /*----- import {{{------*/
if($currentView == News::VIEW_IMAGE_IMPORT):?>
...
<pre>php index.php --tree_id=<?=$tree_id;?> --tag=<?=$tag;?> --news_id=<?=$news_id;?> --class=News --task=import --username=<?=$username;?> --password=&lt;password&gt;</pre>
</p>
...
<?  /*----- resize {{{------*/
if($currentView == News::VIEW_IMAGE_RESIZE):?>
...
<p>
U kunt dit proces ook starten via de shell:
<pre>php index.php --tree_id=<?=$tree_id;?> --tag=<?=$tag;?> --news_id=<?=$news_id;?> --class=News --task=resize --username=<?=$username;?> --password=&lt;password&gt;</pre>
</p>
...
<?  /*-----delete {{{------*/
if($currentView == News::VIEW_IMAGE_DELETE):?>
...
<div class="newsimage">
<?foreach($newsimage['data'] as $item):?>
	<?if($item['thumbnail']):?>
	<a class="newsimg" rel="lightbox[<?=$item['tag'];?>]" href="<?=$item['image']['src'];?>" title="<?=htmlentities($item['name']);?>"><img src="<?=$item['thumbnail']['src'];?>" height="<?=$item['thumbnail']['height'];?>" width="<?=$item['thumbnail']['width'];?>" alt="<?=htmlentities($item['name']);?>" /></a>
	<?endif;?>
<?endforeach;?>
</div>
