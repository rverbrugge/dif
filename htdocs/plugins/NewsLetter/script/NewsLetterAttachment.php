 ...
 * @package Common
 */
class NewsLetterAttachment extends Observer
{
...
/*-------- Helper functions {{{------------*/
	...
	public function getAttachmentList($key)
	{
		$retval = array();
		...
		{
			$extension = Utils::getFileExtension($item['file']);
			$filename = $item['name'].".$extension";
			$file = $path.$item['file']; 
			$retval[] = array('file' => $file, 'name' => $filename, 'mime' => mime_content_type($file));
		}
		return $retval;
	}
//}}}
...
/*-------- DbConnector insert function {{{------------*/
		...
		foreach($searchcriteria as $key=>$value)
		{
			switch($key)
			{
				...
				case 'activated' : 
					...
					$SqlParser->addCriteria(new SqlCriteria('b.nl_active', 1));
					break;
			}
		}
	}
	...
	private function getNextWeight()
	{
		...
		$retval = 0;
		$offset = 10;
		$query = sprintf("select max(att_weight) from newsletter_attachment where att_nl_id = %d", $nl_id);
   ...
   *
	...
	protected function insertFile($id, $values)
	{
		...
		$db = $this->getDb();
		$query = sprintf("update newsletter_attachment set att_file = '%s' where att_id = %d", addslashes($filename), $id['id']);
		$res = $db->query($query);
		if($db->isError($res)) throw new Exception($res->getDebugInfo());
	}
   ...
   *
	...
	protected function handlePreUpdate($id, $values)
	{
		...
		$db = $this->getDb();
		$query = "update newsletter_attachment set att_file = '' where att_id = {$id['id']}";
		$res = $db->query($query);
		if($db->isError($res)) throw new Exception($res->getDebugInfo());
	}
...
/*----- handle http requests {{{ -------*/
	...
	public function handleHttpPostRequest()
	{
		...
		{
			case NewsLetter::VIEW_FILE_OVERVIEW : $this->handleTreeOverview(); break;
			case NewsLetter::VIEW_FILE_NEW :  $this->handleTreeNewPost(); break;
			case NewsLetter::VIEW_FILE_EDIT : $this->handleTreeEditPost(); break;
			case NewsLetter::VIEW_FILE_DELETE : $this->handleTreeDeletePost(); break;
			case NewsLetter::VIEW_FILE_IMPORT : $this->handleImportPost(); break;
			case NewsLetter::VIEW_DETAIL : $this->handleOverview(); break;
		}
	...
	public function handleHttpGetRequest()
	{
		...
		{
			case NewsLetter::VIEW_FILE_OVERVIEW : $this->handleTreeOverview(); break;
			case NewsLetter::VIEW_FILE_NEW : $this->handleTreeNewGet(); break;
			case NewsLetter::VIEW_FILE_EDIT : $this->handleTreeEditGet(); break;
			case NewsLetter::VIEW_FILE_DELETE : $this->handleTreeDeleteGet(); break;
			case NewsLetter::VIEW_FILE_IMPORT : $this->handleImportGet(); break;
			case NewsLetter::VIEW_DETAIL : $this->handleOverview(); break;
		}
	}
//}}}
...
/*------- overview request {{{ -------*/
	...
	*/
	private function handleOverview()
	{
		...
		$nl_id = intval($request->getValue('id'));

		$template = $this->getAttachmentTemplate($nl_id, $tag);
	...
	public function getAttachmentTemplate($nl_id, $tag)
	{
		...
		{
			$searchcriteria = array('nl_id' 	=> $nl_id, 
															'activated' 		=> true);
...
/*------- tree new request {{{ -------*/
	...
	*/
	private function handleTreeNewGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(NewsLetter::VIEW_FILE_NEW);
	...
	private function handleTreeNewPost()
	{
		...
		try 
		{
			...
			viewManager::getInstance()->setType(NewsLetter::VIEW_FILE_OVERVIEW);
...
/*------- tree delete request {{{ -------*/
	...
	*/
	private function handleTreeDeleteGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(NewsLetter::VIEW_FILE_DELETE);
