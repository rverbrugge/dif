 ...
 * @package Common
 */
class SubMenu extends Plugin implements GuiProvider
{
	...
	public function __construct()
	{
		...
		$this->template = array();
		$this->templateFile = "submenu.tpl";
		$this->basePath = realpath(dirname(__FILE__)."/../")."/";

		$this->sqlParser->setSelect('select');
		$this->sqlParser->setTable('submenu', 'a');
...
/*-------- DbConnector insert function {{{------------*/
	...
	public function getDefaultValue($fieldname)
	{
		switch($fieldname)
		{
			...
			case 'type' : return self::TYPE_NORMAL; break;
		}
	}
	...
	private function handleOverview()
	{
		...
		foreach($taglist as $item)
		{
			...
			if(!$template->isCached()) 
			{
				...
				if(!$detail['type'] || ($detail['type'] == self::TYPE_SKIP_TOP && $tree->getParentId($currentId) == $tree->getRootId())) continue;

				$childs = array();
				$childlist = $tree->getChildList($currentId);
				foreach($childlist as $subitem)
				{
					if(isset($subitem['visible']) && !$subitem['visible']) continue;
				...
				if($detail['show_name']) $template->setVariable('submenuname',  $tree->getName($currentId));
