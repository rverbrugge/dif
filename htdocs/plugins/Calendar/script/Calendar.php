...
require_once "CalendarComment.php";

/**
 * Main configuration 
 * @package Common
 */
class Calendar extends Plugin implements GuiProvider, RpcProvider, PluginProvider, CliProvider
{
	...
	const VIEW_DETAIL = "calendar";
	const VIEW_ARCHIVE = "calarch";
	...
	static private $grouptypes 	= array(self::GRP_NONE 		=> 'None', 
																			self::GRP_MONTH 	=> 'Month',
																			self::GRP_YEAR		=> 'Year');
	static private $grouptypelist;
	...
	public function __construct()
	{
		...
		$this->template = array();
		$this->templateFile = "calendar.tpl";
		$this->basePath = realpath(dirname(__FILE__)."/../")."/";

		$this->sqlParser->setSelect('select');
		$this->sqlParser->setTable('calendar_settings', 'a');
		...
		$this->sqlParser->addField(new SqlField('a', 'cal_history', 'history', 'Show history', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_DATE));
		$this->sqlParser->addField(new SqlField('a', 'cal_history_comment', 'history_comment', 'History comment', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING));
		$this->sqlParser->addField(new SqlField('a', 'cal_image_width', 'image_width', 'Breedte afbeelding', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER));
		$this->sqlParser->addField(new SqlField('a', 'cal_image_height', 'image_height', 'Hoogte afbeelding', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER));
		$this->sqlParser->addField(new SqlField('a', 'cal_image_max_width', 'image_max_width', 'maximale breedte afbeelding', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER));
		...
		$this->sqlParser->addField(new SqlField('a', 'cal_template', 'template', 'Template', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING));
		$this->sqlParser->addField(new SqlField('a', 'cal_detail_img', 'detail_img', 'Show thumbnail in details', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_BOOLEAN));
		$this->sqlParser->addField(new SqlField('a', 'cal_comment_order_asc', 'comment_order_asc', 'Order ascending', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_BOOLEAN));
		$this->sqlParser->addField(new SqlField('a', 'cal_comment', 'comment', 'Enable comment', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_BOOLEAN));
		$this->sqlParser->addField(new SqlField('a', 'cal_comment_notify', 'comment_notify', 'Enable comment notification', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_BOOLEAN));
		$this->sqlParser->addField(new SqlField('a', 'cal_comment_title', 'comment_title', 'Comment title', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING));
		$this->sqlParser->addField(new SqlField('a', 'cal_comment_display', 'comment_display', 'Comment display', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER, true));
		$this->sqlParser->addField(new SqlField('a', 'cal_comment_width', 'comment_width', 'Comment width', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER, true));
		$this->sqlParser->addField(new SqlField('a', 'cal_cap_name', 'cap_name', 'Caption name', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING, true));
		$this->sqlParser->addField(new SqlField('a', 'cal_cap_email', 'cap_email', 'Caption email', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING, false));
		$this->sqlParser->addField(new SqlField('a', 'cal_cap_desc', 'cap_desc', 'Caption description', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING));
		$this->sqlParser->addField(new SqlField('a', 'cal_cap_submit', 'cap_submit', 'Caption submit button', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING));
		$this->sqlParser->addField(new SqlField('a', 'cal_cap_back', 'cap_back', 'Caption back link', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_STRING));
...
/*-------- Helper functions {{{------------*/
	...
	static public function getGroupTypeList()
	{
		if(isset(self::$grouptypelist)) return self::$grouptypelist;

		self::$grouptypelist = array();
		foreach(self::$grouptypes as $key=>$value)
		{
			self::$grouptypelist[$key] = array('id' => $key, 'name' => $value);
		}
		return self::$grouptypelist;
	}
...
/*-------- DbConnector insert function {{{------------*/
	...
	public function getDefaultValue($fieldname)
	{
		switch($fieldname)
		{
			...
			case 'group_type' : return self::GRP_NONE; break;
			case 'history_comment' : return 'Event occured in history'; break;
			case 'date_format' : return '%d-%b-%Y'; break;
			...
			case 'comment_display' : return CalendarComment::DISP_FORM_BOTTOM; break;
	...
	public function filterFields($fields)
	{
		...
		$fields['history'] = (array_key_exists('history', $fields) && $fields['history']) ? strftime('%Y-%m-%d', strtotime($fields['history'])) : '';
...
/*------- file request {{{ -------*/
	...
	*/
	private function handleFile()
	{
		...
		$key = $this->director->isAdminSection() ? array('id' => $id) : array('id' => $id, 'active' => true, 'cal_active' => true);
		...
		if(!$this->director->isAdminSection())
		{
			...
			// check if type is not archive. if so, check if file is activated (start stop dates)
		...
		// stupid bastards from microsoft: ie does not like attachment option
...
/*------- conf edit request {{{ -------*/
	...
	*/
	private function handleConfEditGet($retrieveFields=true)
	{
		...
		$template->setVariable('cbo_group', Utils::getHtmlCombo($this->getGroupTypeList(), $fields['group_type'], '...'));
	...
	public function getObject($type)
	{
		...
		switch($type)
		{
			case self::TYPE_DEFAULT : 
				...
				$this->reference[$type] = new CalendarOverview($this);
				break;
			case self::TYPE_HEADLINES :
				require_once("CalendarHeadlines.php");
				$this->reference[$type] = new CalendarHeadlines($this);
				break;
			case self::TYPE_ARCHIVE :
				require_once("CalendarArchive.php");
				$this->reference[$type] = new CalendarArchive($this);
				break;
			case self::TYPE_ATTACHMENT :
				require_once("CalendarAttachment.php");
				$this->reference[$type] = new CalendarAttachment($this);
				break;
			case self::TYPE_IMAGE :
				require_once("CalendarImage.php");
				$this->reference[$type] = new CalendarImage($this);
				break;
			case self::TYPE_COMMENT :
				require_once("CalendarComment.php");
				$this->reference[$type] = new CalendarComment($this);
				break;
			case self::TYPE_SETTINGS :
				require_once("CalendarSettings.php");
				$this->reference[$type] = new CalendarSettings($this);
				break;
			default :
...
/*----- handle rpc requests {{{ -------*/
	...
	public function addComment($search)
	{
		try
		{
			...
			$objCal = $this->getObject(self::TYPE_DEFAULT);
			$objSettings = $this->getObject(self::TYPE_SETTINGS);
			$objComment = $this->getObject(self::TYPE_COMMENT);

			// check if cal exists
			$searchcriteria = array('id' => $search['cal_id']);//, 'activated' => true);
			if(!$objCal->exists($searchcriteria)) throw new Exception("Calendar item does not exist");

			$detail = $objCal->getDetail($searchcriteria);
			...
			$objComment->setSettings($settings);
			$objComment->addComment($search);
			$template = $objComment->getOverview($detail['id']);
			$template->setVariable('calendar', $detail);
