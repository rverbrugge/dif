 ...
 * @package Common
 */
class CalendarAttachment extends Observer
{
	...
	public function __construct($plugin)
	{
		...
		$this->template = array();
		$this->templateFile = "calendarattachment.tpl";
		$this->basePath = realpath(dirname(__FILE__)."/../")."/";

		$this->sqlParser->setSelect('select');
		$this->sqlParser->setTable('calendar_attachment', 'a');
		$this->sqlParser->addField(new SqlField('a', 'att_id', 'id', 'id', SqlParser::getTypeSelect()|SqlParser::PKEY, SqlField::TYPE_INTEGER));
		$this->sqlParser->addField(new SqlField('a', 'att_cal_id', 'cal_id', 'Calender', SqlParser::getTypeSelect()|SqlParser::getTypeModify(), SqlField::TYPE_INTEGER, true));
		...
		$this->sqlParser->addFrom("inner join calendar as b on b.cal_id = a.att_cal_id");
...
/*-------- DbConnector insert function {{{------------*/
	...
	private function getNextWeight()
	{
		...
		$retval = 0;
		$offset = 10;
		$query = sprintf("select max(att_weight) from calendar_attachment where att_cal_id = %d", $cal_id);
   ...
   *
	...
	protected function insertFile($id, $values)
	{
		...
		$db = $this->getDb();
		$query = sprintf("update calendar_attachment set att_file = '%s' where att_id = %d", addslashes($filename), $id['id']);
		$res = $db->query($query);
		if($db->isError($res)) throw new Exception($res->getDebugInfo());
	}
   ...
   *
	...
	protected function handlePreUpdate($id, $values)
	{
		...
		$db = $this->getDb();
		$query = "update calendar_attachment set att_file = '' where att_id = {$id['id']}";
		$res = $db->query($query);
		if($db->isError($res)) throw new Exception($res->getDebugInfo());
	}
...
/*----- handle http requests {{{ -------*/
	...
	public function handleHttpPostRequest()
	{
		...
		{
			case Calendar::VIEW_FILE_OVERVIEW : $this->handleTreeOverview(); break;
			case Calendar::VIEW_FILE_NEW :  $this->handleTreeNewPost(); break;
			case Calendar::VIEW_FILE_EDIT : $this->handleTreeEditPost(); break;
			case Calendar::VIEW_FILE_DELETE : $this->handleTreeDeletePost(); break;
			case Calendar::VIEW_FILE_IMPORT : $this->handleImportPost(); break;
	...
	public function handleHttpGetRequest()
	{
		...
		{
			case Calendar::VIEW_FILE_OVERVIEW : $this->handleTreeOverview(); break;
			case Calendar::VIEW_FILE_NEW : $this->handleTreeNewGet(); break;
			case Calendar::VIEW_FILE_EDIT : $this->handleTreeEditGet(); break;
			case Calendar::VIEW_FILE_DELETE : $this->handleTreeDeleteGet(); break;
			case Calendar::VIEW_FILE_IMPORT : $this->handleImportGet(); break;
			case Calendar::VIEW_DETAIL : $this->handleOverview(); break;
		}
	}
//}}}
...
/*------- overview request {{{ -------*/
	...
	*/
	private function handleOverview()
	{
		...
		if(!$request->exists('id')) throw new Exception('Calender is missing.');
		$cal_id = intval($request->getValue('id'));

		$template = $this->getAttachmentTemplate($cal_id, $tag);
		$this->template[$tag] = $template;
	}

	public function getAttachmentTemplate($cal_id, $tag)
	{
...
/*------- tree overview request {{{ -------*/
	...
	*/
	private function handleTreeOverview()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(Calendar::VIEW_FILE_OVERVIEW);

		if(!$request->exists('cal_id')) throw new Exception('Calender is missing.');
		...
		// create urls
		$url_new = clone $url;
		$url_new->setParameter($view->getUrlId(), Calendar::VIEW_FILE_NEW);
		$template->setVariable('href_new',  $url_new->getUrl(true), false);

		$url_import = clone $url;
		$url_import->setParameter($view->getUrlId(), Calendar::VIEW_FILE_IMPORT);
		$template->setVariable('href_import',  $url_import->getUrl(true), false);

		$url_edit = clone $url;
		$url_edit->setParameter($view->getUrlId(), Calendar::VIEW_FILE_EDIT);

		$url_del = clone $url;
		$url_del->setParameter($view->getUrlId(), Calendar::VIEW_FILE_DELETE);
...
/*------- tree settings {{{ -------*/
	private function handleTreeSettings($template)
	{
		...
		$url->setParameter('tag', $tag);
		$url->setParameter($view->getUrlId(), Calendar::VIEW_FILE_OVERVIEW);
		$template->setVariable('href_back',  $url->getUrl(true), false);

		// add breadcrumb item
		$breadcrumb = array('name' => $view->getName(Calendar::VIEW_FILE_OVERVIEW), 'path' => $url->getUrl(true));
...
/*------- tree new request {{{ -------*/
	...
	*/
	private function handleTreeNewGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(Calendar::VIEW_FILE_NEW);
	...
	private function handleTreeNewPost()
	{
		...
		try 
		{
			...
			viewManager::getInstance()->setType(Calendar::VIEW_FILE_OVERVIEW);
...
/*------- tree edit request {{{ -------*/
	...
	*/
	private function handleTreeEditGet($retrieveFields=true)
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(Calendar::VIEW_FILE_EDIT);
...
/*------- tree delete request {{{ -------*/
	...
	*/
	private function handleTreeDeleteGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(Calendar::VIEW_FILE_DELETE);
...
/*------- import request {{{ -------*/
	...
	*/
	private function handleImportGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(Calendar::VIEW_FILE_IMPORT);
