 ...
 * @package Common
 */
class CalendarComment extends Observer
{
	const VIEW_KEY = 'com';
	const VIEW_ADD = 1;
...
/*----- handle http requests {{{ -------*/
	...
	public function handleHttpPostRequest()
	{
		...
		{
			case Calendar::VIEW_COMMENT_OVERVIEW : $this->handleTreeOverview(); break;
			case Calendar::VIEW_COMMENT_EDIT : $this->handleTreeEditPost(); break;
			case Calendar::VIEW_COMMENT_DELETE : $this->handleTreeDeletePost(); break;
	...
	public function handleHttpGetRequest()
	{
		...
		{
			case Calendar::VIEW_COMMENT_OVERVIEW : $this->handleTreeOverview(); break;
			case Calendar::VIEW_COMMENT_EDIT : $this->handleTreeEditGet(); break;
			case Calendar::VIEW_COMMENT_DELETE : $this->handleTreeDeleteGet(); break;
			default : $this->handleOverview(); break;
		}
	}
//}}}
...
/*------- tree overview request {{{ -------*/
	...
	*/
	private function handleTreeOverview()
	{
		...
		if(!$request->exists('cal_id')) throw new Exception('Nieuwsbericht ontbreekt.');
...
/*------- tree settings {{{ -------*/
	private function handleTreeSettings($template)
	{
		...
		$url->setParameter('cal_id', $cal_id);
		$url->setParameter('tag', $tag);
		$url->setParameter($view->getUrlId(), Calendar::VIEW_COMMENT_OVERVIEW);
		$template->setVariable('href_back',  $url->getUrl(true), false);

		// add breadcrumb item
		$breadcrumb = array('name' => $view->getName(Calendar::VIEW_COMMENT_OVERVIEW), 'path' => $url->getUrl(true));
...
/*------- tree edit request {{{ -------*/
	...
	*/
	private function handleTreeEditGet($retrieveFields=true)
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(Calendar::VIEW_COMMENT_EDIT);
...
/*------- tree delete request {{{ -------*/
	...
	*/
	private function handleTreeDeleteGet()
	{
		...
		$request = Request::getInstance();
		$view = ViewManager::getInstance();
		$view->setType(Calendar::VIEW_COMMENT_DELETE);
